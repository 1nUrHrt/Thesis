...
class AttentionGIN(MessagePassing):
    def __init__(self, node_feature, edge_feature, h_feature, dp_r, heads):
        ...
        self.node_proj = nn.Linear(self.node_feature, self.h_feature) if node_feature != h_feature else nn.Identity()
        self.edge_proj = nn.Linear(self.edge_feature, self.h_feature) if edge_feature != h_feature else nn.Identity()
